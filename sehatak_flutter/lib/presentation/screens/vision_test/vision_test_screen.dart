...
class _VisionTestScreenState extends State<VisionTestScreen> {
  ...
  Widget build(BuildContext context) {
    if (_done) return Scaffold(appBar: AppBar(title: const Text('فحص النظر')), body: Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
      ...
      SizedBox(height: 20), Text('النتيجة: $_score/${_qs.length}', style: const TextStyle(fontSize: 28, fontWeight: FontWeight.bold)),
      Text(_score >= 4 ? '👁️ نظرك جيد!' : '⚠️ ننصح بمراجعة طبيب', style: const TextStyle(fontSize: 16)),
      SizedBox(height: 30), ElevatedButton(onPressed: () => setState(() { _q = 0; _score = 0; _done = false; }), child: const Text('إعادة')),
    ])));
    ...
    return Scaffold(
      ...
      body: Padding(padding: const EdgeInsets.all(20), child: Column(children: [
        ...
        SizedBox(height: 20), Text('سؤال ${_q + 1} من ${_qs.length}', style: const TextStyle(color: AppColors.grey)),
        SizedBox(height: 30),
        ...
        SizedBox(height: 30), Text(q['q'], style: const TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
        SizedBox(height: 20),
