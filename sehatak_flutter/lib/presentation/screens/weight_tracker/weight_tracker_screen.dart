...
class _WeightTrackerScreenState extends State<WeightTrackerScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: SingleChildScrollView(padding: const EdgeInsets.all(14), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Container(padding: const EdgeInsets.all(20), decoration: BoxDecoration(gradient: const LinearGradient(colors: [AppColors.success, AppColors.teal]), borderRadius: BorderRadius.circular(16)), child: Column(children: [const Text('وزنك الحالي', style: TextStyle(color: Colors.white70)), Text('$_current', style: const TextStyle(color: Colors.white, fontSize: 48, fontWeight: FontWeight.bold)), const Text('كجم', style: TextStyle(color: Colors.white70)), SizedBox(height: 8), Text('🎯 الهدف: $_target كجم', style: const TextStyle(color: Colors.white70))])),
        SizedBox(height: 14),
        Row(children: [
          _stat('خسرت', '${_history.isNotEmpty ? _history.last['weight'] : 0}', Icons.trending_down, AppColors.success), SizedBox(width: 8),
          _stat('متبقي', '${(_current - _target).toStringAsFixed(1)}', Icons.flag, AppColors.warning), SizedBox(width: 8),
        ...
        for (final h in _history) Container(margin: const EdgeInsets.only(bottom: 6), padding: const EdgeInsets.all(12), decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(10)), child: Row(children: [const Text('⚖️', style: TextStyle(fontSize: 24)), SizedBox(width: 10), Expanded(child: Text(h['date'], style: const TextStyle(fontWeight: FontWeight.w500))), Text('${h['weight']} كجم', style: const TextStyle(fontWeight: FontWeight.bold, color: AppColors.primary, fontSize: 16))])),
      ])),
  ...
  Widget _stat(String l, String v, IconData i, Color c) => Expanded(child: Container(padding: const EdgeInsets.all(12), decoration: BoxDecoration(color: c.withOpacity(0.06), borderRadius: BorderRadius.circular(12)), child: Column(children: [Icon(i, color: c, size: 20), SizedBox(height: 4), Text(v, style: TextStyle(fontWeight: FontWeight.bold, fontSize: 14, color: c)), Text(l, style: const TextStyle(fontSize: 10, color: AppColors.grey))])));
}
