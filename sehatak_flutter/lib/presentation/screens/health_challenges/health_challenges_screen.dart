          ...
          child: ListView.builder(padding: const EdgeInsets.all(12), itemCount: filtered.length, itemBuilder: (ctx, i) {
            ...
            return Container(
              ...
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                ...
                SizedBox(height: 8),
                Row(children: [Text('${(prog * 100).toInt()}%', style: TextStyle(fontSize: 11, fontWeight: FontWeight.bold, color: c['status'] == 'مكتمل' ? AppColors.success : c['color'])), SizedBox(width: 8), Expanded(child: LinearProgressIndicator(value: prog, backgroundColor: AppColors.surfaceContainerLow, color: c['status'] == 'مكتمل' ? AppColors.success : c['color'], minHeight: 6, borderRadius: BorderRadius.circular(3)))]),
                SizedBox(height: 8),
                Row(children: [
                  Row(children: [const Icon(Icons.people, size: 12, color: AppColors.grey), SizedBox(width: 2), Text(c['participants'], style: const TextStyle(fontSize: 9, color: AppColors.grey))]),
                  const Spacer(),
                  Row(children: [const Icon(Icons.timer, size: 12, color: AppColors.grey), SizedBox(width: 2), Text(c['duration'], style: const TextStyle(fontSize: 9, color: AppColors.grey))]),
                  const Spacer(),
