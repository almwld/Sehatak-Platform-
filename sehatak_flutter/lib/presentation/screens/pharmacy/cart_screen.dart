...
class _CartScreenState extends State<CartScreen> {
  ...
  Widget build(BuildContext c) {
    ...
    return Scaffold(body: Stack(children: [Column(children: [
      _hdr(), Expanded(child: ListView(padding: const EdgeInsets.all(16), children: [
        const Text('المنتجات', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)), SizedBox(height: 8),
        ..._items.map((i) => _item(i)),
        SizedBox(height: 16), _addr(), SizedBox(height: 16),
        const Text('شركة التوصيل', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)), SizedBox(height: 8),
        ..._delivery.asMap().entries.map((e) => _delOpt(e.key, e.value)),
        SizedBox(height: 16),
        const Text('طريقة الدفع', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)), SizedBox(height: 8),
        _payRow(), SizedBox(height: 120),
      ])),
    ]), _bar()]));
  }
  ...
  Widget _ok() => Scaffold(body: Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
    ...
    SizedBox(height: 20), const Text('تم تأكيد طلبك!', style: TextStyle(fontSize: 24, fontWeight: FontWeight.bold)),
    ...
    Expanded(child: Padding(padding: const EdgeInsets.all(12), child: Column(crossAxisAlignment: CrossAxisAlignment.end, children: [
      ...
      Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
        Row(children: [
          ...
          SizedBox(width: 12), Text('${i['qty']}', style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 15)), SizedBox(width: 12),
  ...
  Widget _addr() => Container(padding: const EdgeInsets.all(14), decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(18)), child: Row(children: [const Icon(Icons.location_on, color: AppColors.primary, size: 20), SizedBox(width: 10), Expanded(child: TextField(controller: TextEditingController(text: _address), onChanged: (v) => _address = v, textAlign: TextAlign.right, decoration: const InputDecoration(border: InputBorder.none), style: const TextStyle(fontSize: 13)))]));

  Widget _delOpt(int idx, Map<String, String> co) {
    final sel = _selDelivery == idx;
    return GestureDetector(onTap: () => setState(() => _selDelivery = idx), child: Container(margin: const EdgeInsets.only(bottom: 8), padding: const EdgeInsets.all(14), decoration: BoxDecoration(color: sel ? AppColors.primary.withOpacity(0.05) : Colors.white, borderRadius: BorderRadius.circular(16), border: Border.all(color: sel ? AppColors.primary : Colors.transparent, width: 1.5)), child: Row(children: [Icon(Icons.local_shipping, size: 20, color: sel ? AppColors.primary : AppColors.grey), SizedBox(width: 10), Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.end, children: [Text(co['name']!, style: TextStyle(fontWeight: FontWeight.bold, fontSize: 14, color: sel ? AppColors.primary : null)), Text('خلال ${co['time']}', style: const TextStyle(fontSize: 12, color: AppColors.grey))])), SizedBox(width: 10), Text('${co['price']} ريال', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 14, color: sel ? AppColors.primary : AppColors.darkGrey))])));
  }

  Widget _payRow() => SizedBox(height: 90, child: ListView.separated(scrollDirection: Axis.horizontal, itemCount: _payment.length, separatorBuilder: (_, __) => SizedBox(width: 10), itemBuilder: (_, i) {
    ...
    return GestureDetector(onTap: () => setState(() => _selPayment = p['id']), child: Container(padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 12), decoration: BoxDecoration(color: sel ? (p['color'] as Color).withOpacity(0.1) : Colors.white, borderRadius: BorderRadius.circular(16), border: Border.all(color: sel ? (p['color'] as Color) : AppColors.lightGrey, width: 1.5)), child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
      ...
      SizedBox(height: 5), Text(p['name'], style: TextStyle(fontWeight: FontWeight.w600, fontSize: 11, color: sel ? (p['color'] as Color) : AppColors.darkGrey)),
    ])));
  }));
