    ...
    return Scaffold(
      ...
      body: SingleChildScrollView(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          SizedBox(height: 14),
          Row(children: [_stat('متوسط', '${_avg.toStringAsFixed(1)} س', Icons.bed), SizedBox(width: 8), _stat('أفضل يوم', 'الجمعة', Icons.star), SizedBox(width: 8), _stat('الهدف', '8 ساعات', Icons.flag)]),
          SizedBox(height: 14),
          Container(padding: const EdgeInsets.all(14), decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(14), boxShadow: [BoxShadow(color: Colors.black.withOpacity(0.03), blurRadius: 6)]), height: 160, child: Row(crossAxisAlignment: CrossAxisAlignment.end, children: _week.map((d) => Expanded(child: Column(mainAxisAlignment: MainAxisAlignment.end, children: [Text('${(d['hours'] as double).toStringAsFixed(0)}س', style: const TextStyle(fontSize: 9)), SizedBox(height: 4), Container(width: 26, height: (d['hours'] as double) * 14, decoration: BoxDecoration(color: (d['hours'] as double) >= 8 ? AppColors.success : (d['hours'] as double) >= 6 ? AppColors.info : AppColors.error, borderRadius: BorderRadius.circular(6))), SizedBox(height: 4), Text(d['day'], style: const TextStyle(fontSize: 9, color: AppColors.grey))]))).toList()),),
          SizedBox(height: 14),
