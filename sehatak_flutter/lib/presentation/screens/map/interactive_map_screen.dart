...
class _InteractiveMapScreenState extends State<InteractiveMapScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: Stack(children: [
        ...
        // تصنيفات
        Positioned(top: 10, left: 10, right: 10, child: SizedBox(height: 42, child: ListView.separated(scrollDirection: Axis.horizontal, itemCount: 4, separatorBuilder: (_, __) => SizedBox(width: 6), itemBuilder: (c, i) {
  ...
  Widget _btn(String label, IconData icon, Color color, VoidCallback onTap) {
    return GestureDetector(
      onTap: onTap,
      child: Container(padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 5), decoration: BoxDecoration(color: color.withOpacity(0.1), borderRadius: BorderRadius.circular(8)), child: Row(children: [Icon(icon, size: 12, color: color), SizedBox(width: 2), Text(label, style: TextStyle(fontSize: 9, color: color))])),
    );
  }
  ...
  Widget _trackingCard() {
    ...
    return Container(
      ...
      child: Column(children: [
        Row(children: [Container(width: 36, height: 36, decoration: BoxDecoration(color: AppColors.primary.withOpacity(0.1), borderRadius: BorderRadius.circular(8)), child: const Icon(Icons.local_shipping, color: AppColors.primary, size: 18)), SizedBox(width: 8), const Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text('طلبك في الطريق!', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 13)), Text('رقم: #SHK-784512', style: TextStyle(fontSize: 9, color: AppColors.grey))]))]),
        SizedBox(height: 10),
