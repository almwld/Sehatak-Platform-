      ...
      body: SingleChildScrollView(padding: const EdgeInsets.all(14), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        ..._members.map((m) => Container(
          ...
          child: Row(children: [
            ...
            SizedBox(width: 10),
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(m['name'], style: const TextStyle(fontWeight: FontWeight.bold)), Text('${m['relation']} • ${m['age']} سنة', style: const TextStyle(fontSize: 10, color: AppColors.grey)), Row(children: [Icon(Icons.bloodtype, size: 14, color: AppColors.error), SizedBox(width: 4), Text(m['blood'], style: const TextStyle(fontSize: 11)), SizedBox(width: 10), const Icon(Icons.medical_services, size: 14, color: AppColors.warning), SizedBox(width: 4), Text(m['conditions'], style: const TextStyle(fontSize: 11))])])),
