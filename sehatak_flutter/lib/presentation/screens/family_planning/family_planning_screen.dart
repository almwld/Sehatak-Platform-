  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: SingleChildScrollView(padding: const EdgeInsets.all(14), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        for (final m in _methods) Container(margin: const EdgeInsets.only(bottom: 8), padding: const EdgeInsets.all(14), decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(14), boxShadow: [BoxShadow(color: Colors.black.withOpacity(0.03), blurRadius: 6)]), child: Row(children: [Container(width: 44, height: 44, decoration: BoxDecoration(color: (m['color'] as Color).withOpacity(0.08), borderRadius: BorderRadius.circular(10)), child: Icon(Icons.medical_services, color: m['color'], size: 22)), SizedBox(width: 10), Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(m['name'], style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 14)), Text(m['desc'], style: const TextStyle(fontSize: 10, color: AppColors.grey)), Row(children: [Text('فعالية: ${m['effectiveness']}', style: TextStyle(fontSize: 10, color: m['color'])), SizedBox(width: 10), Text('المدة: ${m['duration']}', style: const TextStyle(fontSize: 10, color: AppColors.grey))])])), const Icon(Icons.arrow_back_ios, size: 14, color: AppColors.grey)])),
        SizedBox(height: 16),
