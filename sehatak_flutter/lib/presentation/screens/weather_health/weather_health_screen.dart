...
class WeatherHealthScreen extends StatelessWidget {
  ...
  Widget _w(String label, String value, String emoji) => Column(children: [Text(emoji, style: const TextStyle(fontSize: 20)), SizedBox(height: 2), Text(value, style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 14)), Text(label, style: const TextStyle(color: Colors.white60, fontSize: 9))]);
  Widget _tip(String emoji, String t, String d, Color c) => Container(margin: const EdgeInsets.only(bottom: 6), padding: const EdgeInsets.all(12), decoration: BoxDecoration(color: c.withOpacity(0.05), borderRadius: BorderRadius.circular(10), border: Border.all(color: c.withOpacity(0.15))), child: Row(children: [Text(emoji, style: const TextStyle(fontSize: 24)), SizedBox(width: 10), Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(t, style: TextStyle(fontWeight: FontWeight.bold, fontSize: 13, color: c)), Text(d, style: const TextStyle(fontSize: 11, color: AppColors.darkGrey))]))]));
}
